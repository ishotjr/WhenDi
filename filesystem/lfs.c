...
static int lfs_dir_append(lfs_t *lfs, lfs_dir_t *dir,
    ...
    while (true) {
        if (dir->d.size + lfs_entry_size(entry) <= lfs->cfg->block_size) {
            ...
            int err = lfs_dir_commit(lfs, dir, region, 2);
            lfs_entry_fromle32(&entry->d);
            return err;
        }
        ...
        if (!(0x80000000 & dir->d.size)) {
            ...
            err = lfs_dir_commit(lfs, dir, region, 2);
            lfs_entry_fromle32(&entry->d);
            if (err) {
                return err;
            }
...
static int lfs_dir_remove(lfs_t *lfs, lfs_dir_t *dir, lfs_entry_t *entry) {
    ...
    int err = lfs_dir_commit(lfs, dir, region, 1);
    if (err) {
        return err;
    }
...
static int lfs_ctz_find(lfs_t *lfs,
    ...
    lfs_off_t tempIndex = {size-1};
    lfs_off_t current = lfs_ctz_index(lfs, &tempIndex);
...
static int lfs_ctz_traverse(lfs_t *lfs,
    ...
    lfs_off_t tempIndex = {size-1};
    lfs_off_t index = lfs_ctz_index(lfs, &tempIndex);
...
int lfs_file_open(lfs_t *lfs, lfs_file_t *file,
    ...
    if (lfs->cfg->file_buffer) {
        file->cache.buffer = (uint8_t*)lfs->cfg->file_buffer;
    } else if ((file->flags & 3) == LFS_O_RDONLY) {
        file->cache.buffer = (uint8_t*)lfs_malloc(lfs->cfg->read_size);
        if (!file->cache.buffer) {
            return LFS_ERR_NOMEM;
        }
    } else {
        file->cache.buffer = (uint8_t*)lfs_malloc(lfs->cfg->prog_size);
        if (!file->cache.buffer) {
            return LFS_ERR_NOMEM;
        }
    }
...
lfs_ssize_t lfs_file_write(lfs_t *lfs, lfs_file_t *file,
    ...
    if (!(file->flags & LFS_F_WRITING) && file->pos > file->size) {
        ...
        while (file->pos < pos) {
            uint8_t tempArray[] = {0};
            lfs_ssize_t res = lfs_file_write(lfs, file, &tempArray, 1);
            if (res < 0) {
                return res;
            }
        }
...
static int lfs_init(lfs_t *lfs, const struct lfs_config *cfg) {
    ...
    if (lfs->cfg->read_buffer) {
        ...
        lfs->rcache.buffer = (uint8_t*)lfs_malloc(lfs->cfg->read_size);
    ...
    if (lfs->cfg->prog_buffer) {
        ...
        lfs->pcache.buffer = (uint8_t*)lfs_malloc(lfs->cfg->prog_size);
    ...
    if (lfs->cfg->lookahead_buffer) {
        ...
        lfs->free.buffer = (uint32_t*)lfs_malloc(lfs->cfg->lookahead/8);
        if (!lfs->free.buffer) {
            return LFS_ERR_NOMEM;
        }
    }
...
int lfs_format(lfs_t *lfs, const struct lfs_config *cfg) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        err = lfs_dir_commit(lfs, &superdir, region, 1);
